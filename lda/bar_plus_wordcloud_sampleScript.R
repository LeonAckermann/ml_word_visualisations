...
# topic_res is output of the func "get_topic_test"
topic_res <- readRDS("typical_topic.rds")
#view(data.frame(topic_res))

# some preprocessing


get_test_preprocessing_1 <- function(test){
...
get_test_preprocessing_2 <- function(test){
...
if (TRUE){
  ...
  if (TRUE){
    ...
    words_df <- words_df %>%
      mutate(category = sapply(words, find_category)) %>% tibble::as_tibble()
    
    # Reshape 'data' dataframe
    temp <- data %>%
      select(category, cohen_d) %>% tibble::as_tibble()
    temp$category <- as.character(temp$category)
    
    
    # Left join 'words_df' with 'data_reshaped'
    words_df <- words_df %>%
      dplyr:::left_join(temp, by = "category")
    
    # !!!! arbitrarily opposite color setting to meet the output logic
    words_df <- words_df %>%
      mutate(color = ifelse(category == "nothing", "red3","lightgreen"))
  }
  ...
  p1 <- ggplot(data[,1:2], aes(x = category, y = cohen_d, 
    ...
    ggwordcloud::geom_text_wordcloud(data = words_df, aes(label = words, area = freq * 1200, size = freq * 3,colour=color,
    ...
    theme(axis.title.y = element_blank(),
          ...
          legend.position = "top") + labs(title = "Cohen's d of topics")  +
    theme(
